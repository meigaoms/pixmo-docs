...
class OpenAI(LLM):
    def __init__(
        ...
        azure_endpoint: None | str = None,
        azure_ad_token_provider: None | str = None,
    ...
    def client(self) -> openai.OpenAI | openai.AzureOpenAI:
        ...
        # )
        other_kwargs["api_version"] = self.api_version
        other_kwargs["azure_endpoint"] = self.azure_endpoint
        other_kwargs["azure_ad_token_provider"] = self.azure_ad_token_provider
        # other_kwargs["api_key"] = self.api_key
        
        return openai.AzureOpenAI(**other_kwargs, **self.kwargs)
        ...
        **kwargs,
        ...
        if _is_chat_model(self.model_name):
            ...
            def get_generated_texts(self, kwargs, optional_kwargs, prompt) -> list[str]:
                ...
                endpoint_selected = random.choice(endpoints)
                
                self_copy = copy.deepcopy(self)
                self_copy.azure_endpoint = endpoint_selected["name"]
                response = self.retry_wrapper(
                    func=self_copy.client.chat.completions.create,
